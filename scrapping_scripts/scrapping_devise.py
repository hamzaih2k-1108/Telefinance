...
try:
    ...
    if optional_data_div:
        ...
        if len(info_blocks) >= 2:
            metals_div = info_blocks[2]  # Le deuxième bloc contient les métaux précieux
            ...
            if table:
                ...
                # Affichage
                print("✅ Données des devises  récupérées")
                print(df)

                # Sauvegarde
                df.to_csv('data_scrapped_csv/devise.csv', index=False, encoding='utf-8-sig')
                print("\n💾 Fichier sauvegardé : devise.csv")
